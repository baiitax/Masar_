...
import { ensureSchema, insertLead } from "@/lib/db";
...
export async function persistLead(lead) {
  // Phase-0 CRM wiring: prefer Postgres (Neon) when configured; the git-ignored
  // JSONL store remains the fallback for environments without a DATABASE_URL.
  try {
    ...
    console.warn("db persist failed, using jsonl fallback:", err.message);
  }
  try {
